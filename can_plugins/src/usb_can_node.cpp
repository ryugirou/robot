...
namespace can_plugins{
  ...
  */
  ...
  bool UsbCanNode::waitForStatus(const ros::Duration& timeout = ros::Duration(2.0))
  {
  ...
  bool UsbCanNode::Open(void)
  {
      ...
      if(_error)
      {
          NODELET_ERROR("failed to open port: %d", _error.value());
  ...
  bool UsbCanNode::Close(void)
  {
      ...
      if(!this->_port->is_open())
      {
          ...
          NODELET_DEBUG("successfully closed serial port : port already closed");
          return false;
      }
      ...
      {
          // wtf
          NODELET_ERROR("failed to CLOSE port: %d", _error.value());
  
          return true;
      }
  
      NODELET_DEBUG("successfully closed serial port");
      return false;
  }
  ...
  void UsbCanNode::onInit(){
    ...
    if(!nh_priv.getParam("port", _port_name))
    {
        ...
        //NODELET_ERROR("value for parameter port is invalid.");
        //exit(-1);
    }
...
PLUGINLIB_EXPORT_CLASS(can_plugins::UsbCanNode, nodelet::Nodelet);
